...
def test_unknown_stage_rule_is_dropped_not_fatal(tmp_path, monkeypatch):
    ...
    (d / "pack.json").write_text(json.dumps({"stages": [
        ...
        {"key": "contacted", "label": "Contacted", "rule": {"type": "status_at_least", "status": "contacted"}},
    ...
    assert [s["key"] for s in stages] == ["new", "contacted"]


def test_stage_with_non_object_rule_is_dropped_not_fatal(tmp_path, monkeypatch):
    """{"rule": "nonsense"} must not crash — rule.get("type") on a str
    raises AttributeError if the shape isn't checked first."""
    d = tmp_path / "bad-rule-shape"
    d.mkdir()
    (d / "pack.json").write_text(json.dumps({"stages": [
        {"key": "new", "label": "New", "rule": {"type": "all"}},
        {"key": "contacted", "label": "Contacted", "rule": {"type": "status_at_least", "status": "contacted"}},
        {"key": "bad", "label": "Bad", "rule": "nonsense"},
    ]}))
    monkeypatch.setenv("VERTICALS_DIR", str(tmp_path))
    monkeypatch.setenv("VERTICAL", "bad-rule-shape")
    vertical.clear_cache()
    stages = vertical.load_pack()["stages"]
    assert [s["key"] for s in stages] == ["new", "contacted"]


def test_stages_not_a_list_falls_back_to_defaults(tmp_path, monkeypatch):
    """{"stages": 5} must not crash — `for s in stages or []` raises
    TypeError: 'int' object is not iterable if the shape isn't checked."""
    d = tmp_path / "stages-not-list"
    d.mkdir()
    (d / "pack.json").write_text(json.dumps({"stages": 5}))
    monkeypatch.setenv("VERTICALS_DIR", str(tmp_path))
    monkeypatch.setenv("VERTICAL", "stages-not-list")
    vertical.clear_cache()
    pack = vertical.load_pack()
    assert pack["stages"] == vertical.DEFAULT_PACK["stages"]


def test_fewer_than_two_valid_stages_falls_back_to_defaults(tmp_path, monkeypatch):
    """The funnel page indexes stages[worstIdx + 1] with no ErrorBoundary —
    a pack that resolves to fewer than 2 valid stages must fall back to
    DEFAULT_PACK's six rather than shipping a funnel that crashes."""
    d = tmp_path / "one-stage"
    d.mkdir()
    (d / "pack.json").write_text(json.dumps({"stages": [
        {"key": "new", "label": "New", "rule": {"type": "all"}},
    ]}))
    monkeypatch.setenv("VERTICALS_DIR", str(tmp_path))
    monkeypatch.setenv("VERTICAL", "one-stage")
    vertical.clear_cache()
    pack = vertical.load_pack()
    assert pack["stages"] == vertical.DEFAULT_PACK["stages"]
...
def test_merge_failure_degrades_to_defaults_belt_and_braces(tmp_path, monkeypatch):
    """Both stage/persona sanitizers guard their own inputs, so nothing
    known reaches the merge loop broken today — this is the belt-and-braces
    backstop: ANY exception raised while merging a pack over DEFAULT_PACK
    must degrade to defaults, not 500 GET /api/vertical, in case a future
    sanitizer bug reintroduces a crash."""
    d = tmp_path / "merge-blows-up"
    d.mkdir()
    (d / "pack.json").write_text(json.dumps({"stages": [
        {"key": "new", "label": "New", "rule": {"type": "all"}},
        {"key": "contacted", "label": "Contacted", "rule": {"type": "status_at_least", "status": "contacted"}},
    ...
    monkeypatch.setenv("VERTICALS_DIR", str(tmp_path))
    monkeypatch.setenv("VERTICAL", "merge-blows-up")

    def boom(_stages):
        raise RuntimeError("simulated sanitizer bug")

    monkeypatch.setattr(vertical, "_sanitize_stages", boom)
    vertical.clear_cache()
    pack = vertical.load_pack()
    assert pack == vertical.DEFAULT_PACK
