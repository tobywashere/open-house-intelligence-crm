...
- [ ] Write failing behavior tests for verified directory rendering after exactly
...
- [ ] Run `../../.venv/bin/python -m pytest backend/tests/test_single_action.py -q`.
- [ ] Implement the bounded candidate with existing contract validation and
  receipt helpers. Render successful receipts directly; never surface model prose.
- [ ] Repeat candidate tests and existing `backend/tests/test_crm_chat.py`.
- [ ] Review candidate safety and record actual results.
...
- [ ] Test using a gateway boundary double for model selection and a real
  TestClient/CRM dispatcher path for invocation. Never connect to provider accounts.
- [ ] Verify a create proposal changes no lead until approved, edited approval is
  persisted, a repeat approval is refused, and a fresh app lifespan reads it back.
- [ ] Verify note and follow-up proposals and denial leave business rows correct.
- [ ] Verify booking approval respects a slot filled after proposal creation.
- [ ] Run the lifecycle tests and record the boundary still requiring real hardware.
...
- [ ] Write failing tests showing a model-selected write never reaches the wrapped
...
- [ ] Implement explicit `--live-read-only` opt-in and bounded localhost requests.
  Use current exported environment settings for tokens, never command arguments.
- [ ] Compare the original lead-directory prompt plus two paraphrases. A successful
  count comparison is not a broad quality benchmark; mark skipped/unavailable paths.
- [ ] Document Bash environment loading and the exact WSL command; explain that
  native plugin configuration must match the target database and remain unchanged.
- [ ] Run all backend tests, native-plugin tests, and whitespace checks. Get an
