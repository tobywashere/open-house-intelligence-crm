...
8. After setting the agent policy, read `agents.list[index].tools` back with `openclaw config get ... --json`. Require its authoritative allowed-tool list to equal exactly `["exec"]`, all known general web/browser/filesystem tools to remain denied, gateway execution to remain allowlist-only, and the effective execution ask mode to equal exactly `off`. Missing, interactive, contradictory, or ambiguous JSON is an unsupported installation and must never produce a ready message.
