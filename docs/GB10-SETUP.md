...
Target state: `http://<gb10>:8000` (over Tailscale) serves the dashboard; the same
...
browser (any teammate, via Tailscale)
   → GB10 :8000  FastAPI  — serves dashboard build + /api + SQLite
        ↕ chat relay (Bearer token)
     GB10 :18789 OpenClaw gateway — Qwen 3.6 35B-A3B
        ↳ skill: skills/tools.py → http://localhost:8000/api
```
...
3. **Install the CRM skill** into the OpenClaw workspace:
   ...
   export CRM_API_URL=http://localhost:8000/api
   ```
...
dashboard itself — **one port, one URL: `http://gb10:8000`**.

## 3. Verify the wiring (in order — each step isolates one link)

| Check | Expect |
|---|---|
| `curl localhost:8000/api/health` | `{"ok":true,"agent_mode":"openclaw","agent_connected":true}` |
| `CRM_API_URL=http://localhost:8000/api python3 -c "import sys;sys.path.insert(0,'skills');import tools;print(tools.list_leads()[0]['name'])"` | a lead name — the skill reaches the backend |
...
at the GB10 instead: `VITE_API_URL=http://gb10:8000/api npm run dev`.
